...
public class ForgeClientPlatformHelper implements ClientPlatformHelper {
    ...
    public <E extends Entity> void registerEntityRenderer(
        ...
        var handler =
                new ForgeClientPlatformHelper.RendererRegistrationHandler<E>(
                        entityType, entityRendererFactory);
        ForgePlatformHelper.getEventBus().addListener(handler::handleEvent);
    }

    private record RendererRegistrationHandler<T extends Entity>(
            RegistryObject<EntityType<T>> type, EntityRendererProvider<T> rendererProvider) {
