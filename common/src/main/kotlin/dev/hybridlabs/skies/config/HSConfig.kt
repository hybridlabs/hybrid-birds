...
data class HSConfig(
    ...
    companion object {
        val CODEC: Codec<HSConfig> = RecordCodecBuilder.create { instance ->
            instance.group(
                Codec.INT.fieldOf("data_version").forGetter(HSConfig::dataVersion),
                EntitySpawnConfig.CODEC.listOf().fieldOf("spawn_configuration").forGetter(
                    HSConfig::entitySpawnConfig),
            ).apply(instance, ::HSConfig)
        }
    }
}